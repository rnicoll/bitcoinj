...
public class BloomFilter extends Message {
    ...
    public synchronized FilteredBlock applyAndUpdate(Block block) {
        ...
        FilteredBlock filteredBlock = new FilteredBlock(block.getParams(), block.getVersion(),
                block.getPrevBlockHash(), block.getMerkleRoot(), block.getTimeSeconds(),
                block.getDifficultyTarget(), block.getNonce(), pmt);
